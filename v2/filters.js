...
function applyFilters(homes) {
  return homes
    .filter((home) => {
      ...
      const availableLotSwitch = document.querySelector(
        "#available-lot-switch"
      );
      if (availableLotSwitch && (home.construction_status === "available-lot" || home.construction_status === "available")) {
    ...
    .filter((home) => {
      ...
      const underConstructionSwitch = document.querySelector(
        "#under-construction-switch"
      );
      if (
        underConstructionSwitch &&
        (home.construction_status === "under-construction" || home.construction_status === "new-construction")
      ) {
    ...
    .filter((home) => {
      ...
      if (
        ...
        document.getElementById("radio").value === "all"
      ) {
...
function getDistanceInMiles(markerLatitude, markerLongitude) {
  ...
  let distance = google.maps.geometry.spherical.computeDistanceBetween(
    ...
    new google.maps.LatLng(markerLatitude, markerLongitude)
  );
  return distance / 1609;
}

function updateMarkers() {
  let homesWithLatAndLong = homes.filter(
    (home) =>
      home.custom_fields.coordinates_group.latitude &&
      home.custom_fields.coordinates_group.longitud
  );
...
function listingImage(home) {
    ...
    } else if (home.sales_status === "pending") {
      return (salesStatusImg =
...
jQuery(
  '#available-lot-switch, #move-in-ready-switch, #under-construction-switch, #price, input[name="state"], #city, #radio'
